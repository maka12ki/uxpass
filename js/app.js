const VERSION = 'uxpass_v091_feedback_patch';
...
const questions = [
  ...
  {id:'q2',type:'compare',area:'UX概論',q:'あるサービスは画面操作が簡単でエラーも少ない。しかし、申込後に確認連絡がなく、利用者から「本当に予約できたか不安」と評価されている。この状況の捉え方として最も適切なのはどれか。',choices:['操作中のユーザビリティは大きな問題ではないが、利用後を含むUXに課題がある','予約後の不安はUIの色や配置を調整すれば必ず解消できる','予約後の連絡は運用プロセスの話なので、UXではなくCXだけの課題として扱う','エラーが少ないため、まずは新機能追加で利用者の満足度を高める'],answer:0,explain:'UXは利用後の安心感も含む。ユーザビリティが一定水準でも、確認連絡や不安解消が弱ければ体験全体の評価は下がる。'},
  {id:'q3',type:'case',area:'HCD',q:'自治体のオンライン申請サービスで、高齢者の利用率が低い。担当者は「文字サイズを大きくすれば解決する」と考えている。HCDの進め方として最も適切なのはどれか。',choices:['実際の利用者・利用環境・つまずき箇所を確認してから改善方針を決める','文字サイズの調整を改善候補に置きつつ、他の要因は後工程で確認する','職員が想定する高齢者像にもとづき、申請画面の構成を先に決める','既存の紙申請と同じ項目順にすれば、オンラインでも利用しやすいと判断する'],answer:0,explain:'文字サイズは改善候補だが、原因とは限らない。端末、入力内容、本人確認、専門用語、家族の支援など利用状況を確認してから要求や改善方針を整理する。'},
  {id:'q4',type:'method',area:'リサーチ',q:'新サービスの改善で、問い合わせログから「申込条件が分からない」という声が多いことが分かった。次に、利用者がどの情報で迷っているかを具体的に把握したい。最も適切な調査はどれか。',choices:['該当利用者に直近の申込検討の流れを聞くインタビューを行う','該当ページを閲覧した人に、条件理解のしやすさをアンケートで確認する','サポート担当者に、よくある問い合わせ内容を分類してもらう','申込条件ページの情報構造を専門家レビューで確認する'],answer:0,explain:'「どの情報で、なぜ迷ったか」を具体化したいので、直近の経験を聞くインタビューが向く。認知度調査や配色比較では、申込条件の理解課題には届きにくい。'},
  {id:'q5',type:'method',area:'リサーチ',q:'業務システムの入力画面で、利用者は「慣れれば問題ない」と答えるが、観察すると入力前に紙のメモや別システムを何度も確認している。次に重視すべき観点はどれか。',choices:['発言だけでなく、実際の作業環境や前後の行動を含めて課題を整理する','入力時間だけを計測し、平均時間が短ければ課題は小さいと判断する','紙メモの内容をそのままヘルプページに掲載して対応する','利用者に再度アンケートを行い、画面の満足度だけを確認する'],answer:0,explain:'発言と行動がずれている。観察から、画面単体ではなく作業環境・前後工程・情報不足を含めて要求を整理する。'},
  {id:'q6',type:'compare',area:'リサーチ',q:'「多くの利用者がどの選択肢を選ぶか」と「なぜその選択肢を選ぶのか」を把握したい。調査の組み合わせとして最も適切なのはどれか。',choices:['アンケートで傾向を把握し、インタビューで理由を深掘りする','インタビューで仮説を作り、その結果だけで全体傾向として扱う','ログ分析で選択率を把握し、理由はチーム内で解釈して決める','アンケートで自由記述だけを集め、選択率は確認しない'],answer:0,explain:'量の傾向はアンケート、理由の深掘りはインタビューが向く。ログや観察も有効だが、理由を断定するには追加確認が必要。'},
  {id:'q7',type:'case',area:'分析・要求定義',q:'調査で「通知がほしい」という要望が複数出た。要求定義として最も適切な進め方はどれか。',choices:['通知が必要になる場面・知りたい内容・失敗したときの影響を整理する','要望が多いので、まず通知機能を主要導線として設計する','通知が不要な人もいるため、通知要望はいったん優先度を下げる','通知のデザイン表現を先に決め、その後に通知条件を調整する'],answer:0,explain:'要求定義では、要望をそのまま機能化せず、背景にある目的や利用場面を整理する。通知が解決策として妥当かも検討する。'},
...
const mockQuestions = [
  {id:'mq1',type:'case',area:'UX概論',q:'会員登録画面の入力項目は少なく、操作エラーもほぼない。しかし登録後、利用者は「次に何をすればよいか分からない」と離脱している。この状況の判断として最も適切なのはどれか。',choices:['登録操作のユーザビリティだけでなく、登録後の導線を含むUXに課題がある','登録画面でエラーが少ないため、UX上の課題は小さいと判断する','登録後の導線はマーケティング施策なので、UX評価とは切り離す','入力項目をさらに減らせば、登録後の離脱も必ず改善する'],answer:0,explain:'登録操作が簡単でも、その後の目的達成につながらなければ体験全体には課題が残る。UXは利用中の画面操作だけでなく、次の行動への安心感や見通しも含む。'},
  {id:'mq2',type:'case',area:'HCD',q:'新しい勤怠システムの導入で、管理部門は「承認ボタンを目立たせたい」と要望している。一方、現場では申請前の入力内容確認に時間がかかっている。HCDの観点で最初に整理すべきことはどれか。',choices:['管理者と申請者それぞれの目的・作業の流れ・つまずきを確認する','管理部門の要望を優先し、承認ボタンの色と大きさを決める','現場の入力確認は教育で解決し、画面設計の対象から外す','承認ボタンを目立たせた案を作り、利用者調査はリリース後に行う'],answer:0,explain:'複数の利用者・関係者がいる場合は、誰のどの目的・作業に課題があるのかを整理する必要がある。声の大きい要望だけで設計を決めない。'},
  {id:'mq3',type:'method',area:'リサーチ',q:'アプリのログで、初回起動後のチュートリアル画面で多くの利用者が離脱していることが分かった。離脱理由を具体的に知るために、次に有効な方法はどれか。',choices:['初回利用者に実際にチュートリアルを使ってもらい、発話や迷いを観察する','全利用者にアプリのブランドイメージを5段階で回答してもらう','チュートリアルの文章量を競合アプリと比較し、短い方を採用する','離脱率が高い画面を削除し、理由の確認は後回しにする'],answer:0,explain:'ログで「どこで」離脱したかは分かるが、「なぜ」は分からない。実際の利用場面を観察すると、理解できない、価値が伝わらない、操作に迷うなどの理由を把握しやすい。'},
  {id:'mq4',type:'compare',area:'分析・要求定義',q:'あるチームが「20代女性向け」と定義しただけでペルソナ作成を完了したとしている。UX設計で不足している観点として最も適切なのはどれか。',choices:['目的・利用状況・行動特性・課題など、設計判断に使える具体性','年齢と性別をさらに細かく分け、統計的に大きい層だけを選ぶこと','その人物の好きな色や流行だけを詳しく決めること','実在の一人をそのまま採用し、他の利用者像を考えないこと'],answer:0,explain:'ペルソナは属性ラベルではなく、関係者が設計判断に使える利用者像である。目的や状況、課題がないと、シナリオや要求定義に結びつきにくい。'},
  {id:'mq5',type:'case',area:'設計・評価',q:'申込フォームの改善案について、チームは完成度の高い画面デザインを作る前に、情報の順番が理解されるか確認したい。適切な進め方はどれか。',choices:['ワイヤーフレームや低忠実度プロトタイプでタスクを試してもらう','最終デザインまで作り込み、色や余白の印象を先に評価する','開発を完了させてから、問い合わせ数だけで判断する','関係者の好みに合う案を選び、そのまま実装する'],answer:0,explain:'情報の順番や流れを確認したい段階では、低忠実度のプロトタイプで十分検証できる。作り込む前に学ぶことで手戻りを減らせる。'},
  {id:'mq6',type:'compare',area:'設計・評価',q:'開発途中で改善点を見つけるための評価と、公開後に目標達成度を確認する評価を区別したい。適切な説明はどれか。',choices:['形成的評価は改善のため、総括的評価は成果確認のために行う','形成的評価は公開後だけ、総括的評価は設計初期だけに行う','形成的評価は数値を扱わず、総括的評価は利用者を見ない','形成的評価と総括的評価は同じ意味で、区別する必要はない'],answer:0,explain:'形成的評価は設計途中で改善に使う評価。総括的評価は完成度や目標達成を確認する評価。どちらもUX活動では重要だが目的が異なる。'},
  {id:'mq7',type:'case',area:'組織導入',q:'サポート部門には問い合わせが多いが、分析すると原因は営業資料とWebサイトの説明が異なることだった。UX改善として適切な対応はどれか。',choices:['部門横断で接点ごとの説明をそろえ、利用者が迷う原因を減らす','サポート部門の応答速度だけを改善し、他部門の説明は変更しない','営業資料は社内向けなので、利用者体験には影響しないと判断する','問い合わせを減らすため、問い合わせ窓口を見つけにくくする'],answer:0,explain:'UXは複数接点で形成される。Web、営業、サポートの説明がズレると利用者は不安になるため、組織横断で整える必要がある。'},
  {id:'mq8',type:'case',area:'リサーチ',q:'アンケートで満足度は高いが、継続利用率は低い。次に確認すべきこととして最も適切なのはどれか。',choices:['満足と答えた理由だけでなく、実際の利用頻度・利用場面・離脱理由を確認する','満足度が高いので、継続利用率の低下はUXとは関係ないと判断する','アンケート結果を優先し、ログやインタビューは確認しない','継続利用率を上げるため、通知頻度を一律に増やす'],answer:0,explain:'満足度の自己申告と行動指標がズレることはある。定量データと定性調査を組み合わせて、なぜ継続しないのかを確認する。'}
];
...
function reset(){ if(confirm('v0.9.1の進捗を初期化しますか？')){ localStorage.removeItem(VERSION); sessionStorage.clear(); location.href='index.html'; } }
...
function chapterDisplay(c,s){ const current=currentChapter(s); const p=chapterProgress(c,s); if(c.id===current.id){ const curId=lessons[Math.min(s.lesson,lessons.length-1)].id; const pos=Math.max(1,c.lessons.indexOf(curId)+1); return {label:`現在 ${pos}/${c.lessons.length}`, barPct:pct(pos-1,c.lessons.length), done:p.done, total:p.total}; } if(p.done>=p.total) return {label:'完了', barPct:100, done:p.done, total:p.total}; return {label:`${p.done}/${p.total}`, barPct:p.pct, done:p.done, total:p.total}; }
...
function passRate(s){ const st=answerStats(s); const lessonScore=pct(s.done.length,lessons.length); const confidence=Math.min(st.answered/30,1); const accuracyPart=(st.answered?st.accuracy:0)*confidence; return Math.max(0, Math.min(99, Math.round(lessonScore*0.5 + accuracyPart*0.5))); }
function passNote(s){ const st=answerStats(s); if(st.answered===0) return '問題を解くと精度UP'; if(st.answered<10) return `測定中（${st.answered}/10問）`; return '問題結果を反映中'; }
...
function renderHome(){
  ...
  <section class="card"><div class="section-head"><b>学習ロードマップ</b><span class="small">全${chapters.length}章 / ${lessons.length}レッスン</span></div>${chapters.map(c=>{const p=chapterDisplay(c,s); return `<div class="road-row ${c.id===ch.id?'now':''}"><div><b>${c.title}</b><p>${c.goal}</p><span class="small">完了 ${p.done}/${p.total}</span></div><span>${p.label}</span></div><div class="mini-meter"><div style="width:${p.barPct}%"></div></div>`}).join('')}</section>
  <section class="grid"><a class="mini" href="quiz.html"><b>${st.accuracy || '-'}%</b><span class="small">直近正答率</span></a><a class="mini" href="profile.html"><b>${passRate(s)}%</b><span class="small">合格予測<br>${passNote(s)}</span></a><a class="mini" href="review.html"><b>${s.wrong.length}</b><span class="small">復習待ち</span></a><a class="mini" href="mock.html"><b>β</b><span class="small">ミニ模試</span></a></section>
...
function renderStudy(){
  const s=getState(); const idx=Math.min(s.lesson,lessons.length-1); const l=lessons[idx]; const ch=chapters.find(c=>c.title===l.chapter); const cp=chapterProgress(ch,s); const curPos=ch.lessons.indexOf(l.id)+1; const isChapterEnd=curPos===ch.lessons.length;
  document.body.innerHTML=`<main class="app">${header()}
  <section class="card"><div class="small">${l.chapter} / ${idx+1} of ${lessons.length}</div><h1 class="lesson-title">${l.title}</h1><div class="meter"><div class="bar" style="width:${pct(idx,lessons.length)}%"></div></div><div class="map-note"><b>この章の位置づけ</b><br>${ch.goal}<br><span>現在 ${curPos}/${ch.lessons.length}・完了 ${cp.done}/${cp.total}</span></div><p class="content lead">${l.intro}</p><p class="content">${highlight(l.body,l.terms)}</p><div>${l.terms.map(t=>`<button class="tag tagbtn" onclick="openTerm('${t}')">${t}</button>`).join('')}</div></section>
  ${isChapterEnd ? `<section class="card"><div class="section-title" style="margin-top:0">章末チェック</div><p class="content">この章で学んだ内容をまとめて確認します。次の章へ行く前の軽いテストです。</p><button class="btn" onclick="startChapterQuiz('${l.chapter}')">章末チェックへ</button></section>` : `<section class="card soft"><b>理解チェックは章末でまとめて</b><p class="content">このレッスンでは流れを優先します。章の最後にケース問題でまとめて確認します。</p></section>`}
  <button class="btn secondary" onclick="completeLesson()">${isChapterEnd?'チェックはあとで、次の章へ':'次のレッスンへ'}</button></main>${nav('学習')}`;
}
function startChapterQuiz(chapter){ sessionStorage.setItem(`chapterQidx_${chapter}`,'0'); location.href=`quiz.html?chapter=${encodeURIComponent(chapter)}`; }
...
function getQuizPool(){
  ...
  if(params.get('mode')==='mock') return mockQuestions;
  if(params.get('mode')==='random') return questions;
  if(params.get('area')) return questions.filter(q=>q.area===params.get('area'));
  if(params.get('chapter')) return questions.filter(q=>q.area===params.get('chapter'));
...
function renderQuizStart(){
  ...
  document.body.innerHTML=`<main class="app">${header()}<section class="hero"><h1>問題演習</h1><p class="sub">章・ジャンルを選んで取り組めます。迷ったらランダムでOK。</p><button class="btn" onclick="sessionStorage.setItem('randomQidx','0');location.href='quiz.html?mode=random'">ランダムで解く</button></section><section class="card"><b>章から選ぶ</b>${areas.map(a=>`<button class="quiz-option" onclick="sessionStorage.setItem('areaQidx_${a}','0');location.href='quiz.html?area=${encodeURIComponent(a)}'">${a}</button>`).join('')}</section></main>${nav('問題')}`;
}
function renderQuiz(){
  const params=new URLSearchParams(location.search); const mode=params.get('mode'); const lesson=params.get('lesson'); const area=params.get('area'); const chapter=params.get('chapter');
  if(!mode && lesson===null && !area && !chapter) return renderQuizStart();
  const pool=getQuizPool();
  let key= mode==='mock' ? 'mockQidx' : mode==='random' ? 'randomQidx' : area ? `areaQidx_${area}` : chapter ? `chapterQidx_${chapter}` : `lessonQidx_${lesson}`;
  let qi=Number(sessionStorage.getItem(key)||0);
  if(mode==='mock' && qi>=pool.length) return renderMockResult();
  if((chapter || lesson!==null) && qi>=Math.min(5,pool.length)){ let s=getState(); if(lesson!==null){ const li=Number(lesson); if(!s.done.includes(li)) s.done.push(li); s.lesson=Math.max(s.lesson, Math.min(li+1, lessons.length-1)); } if(chapter){ const currentIdx=Math.min(s.lesson, lessons.length-1); if(!s.done.includes(currentIdx)) s.done.push(currentIdx); s.lesson=Math.min(currentIdx+1, lessons.length-1); } setState(s); return location.href='study.html'; }
  qi=qi%pool.length; const q=pool[qi]; const order=shuffle(q.choices.map((c,i)=>({c,i}))); window.currentQ=q; window.quizKey=key; window.quizAnswered=false; window.quizMode=mode||'';
  const label=mode==='mock'?'ミニ模試':chapter?'章末チェック':lesson!==null?'レッスン確認':area?`${area} 演習`:'ランダム演習';
  document.body.innerHTML=`<main class="app">${header()}<section class="card"><div class="small">${label} / ${q.area} / ${q.type}</div><h1 class="lesson-title">Q${qi+1}${mode==='mock'?` / ${pool.length}`:''}</h1><p class="content question-text">${q.q}</p><div id="opts">${order.map(o=>`<button class="quiz-option" onclick="answer(${o.i},this)">${o.c}</button>`).join('')}</div><div id="result"></div></section><button id="nextBtn" class="btn secondary" onclick="nextQ()" disabled>回答すると次へ進めます</button></main>${nav('問題')}`;
}
function answer(i,btn){ const q=window.currentQ; if(window.quizAnswered) return; window.quizAnswered=true; const correct=i===q.answer; document.querySelectorAll('.quiz-option').forEach(b=>b.disabled=true); btn.classList.add(correct?'correct':'wrong'); recordAnswer(q,correct); if(window.quizMode==='mock'){ const arr=JSON.parse(sessionStorage.getItem('mockResults')||'[]'); arr.push({id:q.id,correct,area:q.area}); sessionStorage.setItem('mockResults',JSON.stringify(arr)); } const nb=document.getElementById('nextBtn'); if(nb){ nb.disabled=false; nb.textContent='次の問題'; } document.getElementById('result').innerHTML=`<div class="explain"><b>${correct?'正解。':'惜しい。'}</b><br>${q.explain}<br><br><b>正答</b><br>${q.choices[q.answer]}</div>`; }
function nextQ(){ if(!window.quizAnswered){ showToast('先に選択肢を選んでください'); return; } const key=window.quizKey||'freeQidx'; sessionStorage.setItem(key,Number(sessionStorage.getItem(key)||0)+1); const params=new URLSearchParams(location.search); if(params.get('mode')==='mock') location.href='quiz.html?mode=mock'; else if(params.get('mode')==='random') location.href='quiz.html?mode=random'; else if(params.get('area')) location.href=`quiz.html?area=${encodeURIComponent(params.get('area'))}`; else if(params.get('chapter')) location.href=`quiz.html?chapter=${encodeURIComponent(params.get('chapter'))}`; else if(params.get('lesson')!==null) location.href=`quiz.html?lesson=${params.get('lesson')}`; else location.href='quiz.html'; }

function renderMock(){ sessionStorage.removeItem('mockQidx'); sessionStorage.removeItem('mockResults'); document.body.innerHTML=`<main class="app">${header()}<section class="hero"><h1>ミニ模試 β</h1><p class="sub">まずは8問。通常問題とは別の模試用ケースで、本番っぽい判断を確認します。</p><button class="btn" onclick="sessionStorage.setItem('mockQidx','0');sessionStorage.setItem('mockResults','[]');location.href='quiz.html?mode=mock'">開始する</button></section><section class="card"><b>注意</b><p class="content">まだ100問模試ではありません。選択しないと次へ進めないように修正済みです。</p></section></main>${nav('模試')}`; }
function renderMockResult(){ const s=getState(); const results=JSON.parse(sessionStorage.getItem('mockResults')||'[]'); const correct=results.filter(r=>r.correct).length; const rate=pct(correct, mockQuestions.length); document.body.innerHTML=`<main class="app">${header()}<section class="hero"><h1>ミニ模試終了</h1><p class="sub">今回の正答率：${rate}%（${correct}/${mockQuestions.length}問）<br>現在の合格予測：${passRate(s)}%</p><button class="btn" onclick="sessionStorage.removeItem('mockQidx');sessionStorage.removeItem('mockResults');location.href='mock.html'">もう一度</button></section><section class="card"><b>次に見るところ</b><p class="content">合格予測は、レッスン進捗と問題の回答数・正答率を重みづけして算出します。少数回答だけでは高くなりすぎないよう調整しました。</p></section></main>${nav('模試')}`; }
...
function renderProfile(){ const s=getState(); const st=answerStats(s); const byArea={}; Object.entries(s.answers||{}).forEach(([id,a])=>{ byArea[a.area]=byArea[a.area]||{n:0,c:0}; byArea[a.area].n++; if(a.lastCorrect) byArea[a.area].c++; }); const weak=Object.entries(byArea).map(([area,v])=>({area,rate:pct(v.c,v.n),n:v.n})).sort((a,b)=>a.rate-b.rate).slice(0,3); document.body.innerHTML=`<main class="app">${header()}<section class="hero"><h1>記録</h1><p class="sub">完了 ${s.done.length}/${lessons.length} レッスン<br>直近正答率 ${st.accuracy||0}% / 回答済み ${st.answered}問</p><div class="meter"><div class="bar" style="width:${pct(s.done.length,lessons.length)}%"></div></div></section><section class="grid"><div class="mini"><b>${passRate(s)}%</b><span class="small">合格予測<br>${passNote(s)}</span></div><div class="mini"><b>${s.wrong.length}</b><span class="small">復習待ち</span></div><div class="mini"><b>${st.totalAttempts}</b><span class="small">総回答数</span></div><div class="mini"><b>${s.xp}</b><span class="small">XP</span></div></section><section class="card"><b>苦手候補</b>${weak.length?weak.map(w=>`<div class="row"><span>${w.area}</span><b>${w.rate}%</b></div>`).join(''):'<p class="content">まだ問題数が少ないため、もう少し解くと表示されます。</p>'}</section><button class="btn ghost" onclick="reset()">進捗を初期化</button></main>${nav('記録')}`; }
